...
def get_provider_info(provider_info={}):
    ...
    if not provider_info.get('platform'):
        ...
        platform = require_input("Select a platform type (1/2): ", lambda answer: answer if answer in ['1','2'] else None, default='1')
        if platform == '1':
            platform = KVM
        elif platform == '2':
            platform = XEN
...
def get_provider_credentials(credential_info={}):
    ...
    if not credential_info.get('ex_force_auth_version'):
        ...
        ex_force_auth_version = require_input("ex_force_auth_version for the provider: ", lambda answer: answer if answer in ['2.0_password','3.x_password'] else None, default='2.0_password')
